...
store.subscribe(() => {
    ...
    todos.length > 0 &&
        todos.forEach((todo) => {
            ...
            todo.completed ? li.classList.add("active") : li.classList.remove("active");
            btnRem.setAttribute("data-id", todo.id);
            li.setAttribute("liid", todo.id);
...
addBtn.addEventListener(
    ...
    () => {
        ...
        store.getState().some(todo => todo.title === todoText.value) ? alert("Task Already added") : todoText.value !== "" &&
            store.dispatch({
                type: ADD_TODO,
...
app.addEventListener("click", (e) => {
    const id = e.target.dataset.id;
    const liid = e.target.getAttribute("liid");

    e.target.type === "button" ? id !== undefined && store.dispatch({ type: REM_TODO, payload: id }) : liid !== undefined && store.dispatch({ type: TOGGLE_TODO, payload: liid })
});
